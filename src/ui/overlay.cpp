...
// render backends take them directly — ImGui_ImplWGPU_* the wgpu device/pass,
// ImGui_ImplVulkan_* the VkInstance/VkDevice/VkCommandBuffer. The overlay's
// own INTERFACE (overlay.h) speaks only rhi::, so nothing above it cares which
// backend ImGui is on; the branch below is decided once by device.Kind().
#include "gpu/rhi_dawn.h"
#include "gpu/rhi_vk.h"
#include "gpu/rhi_vulkan.h"

namespace {
// Which ImGui render backend Init selected; every later call branches on it.
bool g_vulkan = false;
}  // namespace
...
bool Overlay::Init(GLFWwindow* window, const rhi::Device& device,
  ...
  g_vulkan = device.Kind() == rhi::BackendKind::Vulkan;
  if (g_vulkan) {
