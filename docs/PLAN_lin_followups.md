...
| W2-D far blocker bit | DONE 05:52Z, merged into `lin-followups` @ 166fb41 | `worktree-agent-af184e07319093ded` | Far byte = 7-bit material + bit 7 "pristine worldgen puts a blocker in this cell's footprint" (column closed form: ground/fluid top, ruin pad, arena deck; trees excluded because their only bound is 17.5 m wide). `far`/`fardown` agree by construction (both from the procgen column) — so an EDIT never sets the flag. **Kill criterion fired for the hit test:** it recovers half-missing surface cells at 25–205 m but buries single-cell ground cover and paints flat facets; shipped `render.farBlockerHitLevel=0`. The shadow half is kept: distant mountain faces go from ant-trail dither to coherent terrace shading; far march −10% noon, flat dusk; hash unmoved; `far-fog` gate gained a blocker-flag arm; `check_invariants.py` gained `far cell material bits` (materials ≤ 127; 117 today). |
| W3-P0 openness grid | DONE 2026-09-02, worktree `agent-af7256f786d13e256` | baa22ba, 317621b, 7cc48b1 | 12 MiB byte grid per (slot, 4^3 block, face) + 128 KiB per-slot world-chunk stamp; `sim_openness.wgsl` on the TICK table (dirty list + 256 slots/tick refresh), marching `traceOpaque` in coarse mode from t=0 rather than a second DDA. `ambientAt` and `ambientAtP` (microbody) multiply the hemisphere ambient by it. Hash unmoved `01dc3219`; 19/19 smoke probes match under sync validation. Cost: pass p50 0.015 ms idle / 0.008 ms flying / 0.19-0.30 ms under activity (`--perf` node), per-hit read 0.89 ms of 12.25 at noon (`--render-budget` `noopenness`). **The lesson:** the first version wrote 0 ("fully enclosed") for a face whose neighbouring block held any blocker, and at 40 cm blocks over 10 cm voxels that is indistinguishable from a one-voxel terrace step -- every riser on every hillside went hard black (-12.5/255 mean, 43% of pixels). It writes "no opinion" now. Deviations: a MULTIPLY not `mix(ground, sky, openness)`; the pass is on the tick table not the per-frame shadow table; `debris.wgsl`'s per-VERTEX cube and sprite paths are not wired. |
| Infra | main @ a460282 | | `build.sh` heartbeats the lock; both scripts release only as owner. |
| T0 land P0 | DONE 2026-09-02 08:03Z, main @ 49bfce6 (ff) | | `openness`/`determinism`/`shadow-cache`/`sleep` PASS in one boot, hash 01dc3219 unmoved, `--vk-smoke-loud --vk-validation` 19/19 clean. Main rebuilt. |
| T1 white screen | DONE, `lin-followups` | 299d6f1 | **Measured, not bisected:** a startup timeline on stderr (`StartupMark`, wall + process CPU per phase) puts every phase before the frame loop at 5.4 s and frame 1 at 45.5 s wall / 43.6 s CPU (single-threaded) — the lazily created graphics pipelines, i.e. the driver compiling the raymarch FS. `--telemetry` 52.6 s (RENDER_STATS is a different SPIR-V). The 2026-08-31 binary, genuinely cold: ~38 s, so startup is roughly what it always was — but the app's pipeline cache file was NEVER written: only from a shutdown path that a clean `--frames 60` exit did not reach, let alone the tuner's taskkilled Play instances (main's file was 12 h stale with dozens of launches between). **Fix:** `vk::Backend::SavePipelineCache()` called from `EnsureRenderPipelines` the moment the pipelines exist (write-then-rename). Verified: cold run writes the file at frame 1; the next launch builds its render pipelines in 0.05 s and presents frame 1 at 2.7 s. The "12 cores" was not reproduced on this tree (1 core here) — but see T5.1: the worldgen pipeline compile with non-identity curve knots ran past 15 min at 11.5 GB, and W1-C's curve code landed the day the user saw the stall; a driver compile of that kernel is the likely mechanism. Ruin pads / far fill / mirror warm-up: not implicated (all inside the 5.4 s). |
| T2 P0 verdict | DONE | in 299d6f1 | `PLAN_gi.md` §2 "Verdict". Caves/rooms dark, doorway gradient smooth (no tiling), nothing wrongly darkens; the interior is too dark at 1.0 and that is P1's job, default kept. `debris.wgsl` wired per vertex (`renderBGL_` 1/17/18 now Vertex\|Fragment). |
| T3 P1 | DONE | 299d6f1 | `PLAN_gi.md` §3 status + verdict, `DESIGN.md` §9.y. 48 MiB RGB9E5 grid, EMA injection from the resolve pass and the openness walk, nine-ray solid-angle gather. `gi-bounce` PASS; hash unmoved; smoke 19/19 clean; `nogi` 1.20 ms of 11.48 at noon; raymarch FS 128 regs, spill 64→80 B. `giStrength` 2.0 after the eye test. `shadow-cache` pins gi 0 for its arms. `--shot` renders 4 frames per camera and writes the sun before its ticks. |
| T4 P2+P3 | DONE | c80850a | Write-back at `giFeedback` 0.2 (`giStrength ≤ 3` guarded); gate convergence arm 0.0301 → 0.0301. Emission rides `irrSample`; `heatSpill` and its knob deleted; lava spatter at night lights the meadow around it. **Stale fixture found:** the authored lava pool frames look at buried cameras (`PLAN_gi.md` §5). |
| T5.1 armor-react pad + curves | PAD DONE, CURVES STAY IDENTITY | | `bath` stamps a two-voxel stone pad flush over the highest column under both creatures and clears 16 voxels above it, then spawns on it — the gate no longer depends on what procgen puts under WindowOrigin + 250. An authored set (meadow ×0.85, pine S-curve, desert ×0.95) was then tried and REVERTED: with non-identity knots the driver's compile of the worldgen pipelines ran past 15 min at 11.5 GB host memory (killed; the launch sat at "sim init" with worldgen's SPIR-V freshly written). The identity knots const-fold the integer Hermite away; authored ones do not. Before a non-identity default can ship the curve must become a uniform, not a compile-time constant of that kernel (`tuning_params.def`, the curve block). No hash move, no rebaseline. |
| T5.2 god rays | DONE, target NOT met | | One occlusion ray per 4³ block per pixel instead of per sample (`occBlock`/`occHit` in `godRays`: the march is coarse from its first step, so two samples in one block walk the same mask cells); deterministic, time-free, no new dither. `--render-budget --budget-cams submerged` (exclusive, no stray): baseline 30.18 → **27.18 ms**, `godshadow0` 7.93 → **5.62 ms**, `nogodray` 7.32 ms. Still above 25; what remains is the 14 samples themselves — halving them with a per-pixel blue-noise offset is the untried half of the idea. |
| T5.3 mist past the silhouette | TRIED, REMOVED | | An opaque near-field hit probed three cells along its face normal for a CA liquid and drew that liquid's veil. By eye it worked (`--shot-waterfall`: the veil continued onto the cliff face beside the column and the basin lip). Cost, `nomist` arm at noon: **0.96 ms of 11.90 (8.1%)** — it ran on every opaque pixel, and there is no cheap "near water" gate (a meadow chunk's grass is non-blocker cells, so the occupancy word cannot tell). Removed; the `nomist` arm stays as the instrument; the comment at the old site says what it cost. |
| T5.4 far blocker sentence | DONE | | `ARCH_NODES` far-field node. |
