...
// that stack. Cover rows: minY / maxY / patchThreshold ARE enforced
// (worldgen.wgsl cover block, WM_C_MIN_Y / MAX_Y / PATCH_THRESH); maxSlope is
// packed but not enforced (Col has no slope until the landform field lands);
// nearWater* is not even packed. Tree rows: nothing per row reaches the
// shader — only the SPECIES FILE's band/slope (TA_S_MIN_Y / MAX_Y / MAX_SLOPE)
// gate a pick. Water and cave rows: not packed at all.
...
const COND_TREE = 'per-row tree conditions never reach the shader; only the species file’s band and slope gate a pick (treeInfoAt). P-D makes the row’s own compare live.';
const COND_COVER_SLOPE = 'packed (WM_C_MAX_SLOPE) but not enforced: the cover block has no slope until P-D reads the landform field.';
const COND_COVER_WATER = 'nearWater is not packed; P-D adds the pondNear compare.';
...
export const LIVE = {
  ...
  biome: Object.assign({
    ...
    'trees.tile': N('P-D', 'TREE_TILE is one global lattice (worldgen.treeTile); the packed per-biome tile is informational. P-D thins one fine lattice to this spacing'),
    'trees.density': R('WM_B_TREE_DENSITY (percent of tiles)'),
  ...
  conditions('cover.plants[]', ['minY', 'maxY', 'patchThreshold'], 'P-D', COND_COVER_SLOPE),
  conditions('trees.species[]', [], 'P-D', COND_TREE),
