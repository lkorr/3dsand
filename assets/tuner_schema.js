...
const TUNING_SCHEMA = [
...

  {
    ...
    apply: 'shader',
    group: 'sim',
    blurb: 'The experimental MLS-MPM particle liquid (mpm tool: Tab to it, hold LMB to pour, keys 1–4 pick the species, U clears). All integers — Q16.16 fixed point where noted (65536 = 1.0) — feeding a deterministic solver. None of these touch the WORLD hash (the fluid never writes voxels), but every one changes the fluid-det gate hash. Pressure comes from a real density sample each substep: pressure = stiffness × ((density∕rest)^power − 1), floored at −cohesion.',
    params: [
      {k:'fluidStiffness', n:'stiffness', d:'EOS stiffness, Q16.16. Higher resists compression harder and splashes sharper; the incompressibility knob.', min:0, max:3145728, step:16384, int:true},
      {k:'fluidRestDensity', n:'rest density', d:'Particle masses per cell at equilibrium, Q16.16. The pour lays down 8 per cell, so 524288 (8.0) means a fresh pool is exactly at rest. Lower = the same particles spread thinner; higher = they pack tighter before pushing back.', min:65536, max:2097152, step:32768, int:true},
      ...
      {k:'fluidCohesion', n:'cohesion', d:'Surface tension: how much NEGATIVE pressure under-dense fluid may pull with, Q16.16. 0 = droplets never pull together; high values bead the fluid into blobs and hold a pool’s surface tight.', min:0, max:1048576, step:1638, int:true},
      {k:'fluidAttractSame', n:'attract same', d:'Extra pull toward fluid of the SAME species, Q16.16 and signed. Positive makes a species cohere and fuse with itself; negative makes it fizz apart.', min:-524288, max:524288, step:1638, int:true},
      {k:'fluidAttractDiff', n:'attract different', d:'Pull toward OTHER species, Q16.16 and signed. Negative (the default) makes different liquids repel — they layer and sit against each other instead of interpenetrating. Positive makes them eagerly mix.', min:-524288, max:524288, step:1638, int:true},
      {k:'fluidViscosity', n:'viscosity', d:'Dynamic viscosity, Q16.16: resistance to shearing flow. 0 is splashy water; high values pour like honey and kill sloshing fast.', min:0, max:524288, step:1638, int:true},
      {k:'fluidDamping', n:'damping', d:'Fraction of velocity lost per tick, Q16.16 (0..0.9). Non-physical settle aid — a touch of it calms residual jitter in a pool that should be still.', min:0, max:58982, step:655, int:true},
      {k:'fluidGravity', n:'gravity', d:'Fall acceleration, Q16.16 cells per tick squared. 7144 is 9.81 m/s² at 10 cm voxels and 30 Hz.', min:0, max:131072, step:64, int:true},
    ],
  },

  {
    ...
    apply: 'shader',
    group: 'render',
    blurb: 'How the MPM liquid is drawn. One instanced cube per particle — these knobs are what make thousands of cubes read as one connected fluid: oversize so neighbours fuse, stretch so motion streaks, density shading so pressure is visible, foam so spray whitens. All render-only.',
    params: [
      {k:'fluidColor', n:'species 1 colour', d:'Albedo of species 1 (key 1 while the mpm tool is active). The water-ish default.', type:'vec3', min:0, max:1, step:0.01},
      ...
      {k:'fluidParticleSize', n:'particle size', d:'Cube half-extent in cells. 0.5 exactly tiles the rest lattice; a little over closes the gaps so a pool reads as a surface instead of a bag of dice.', min:0.2, max:1.2, step:0.01},
      {k:'fluidStretch', n:'motion stretch', d:'How much a particle elongates along its velocity. Free motion blur: falling streams read as streaks rather than separated droplets.', min:0, max:1.5, step:0.01},
      {k:'fluidDensityShade', n:'pressure shading', d:'Albedo darkening where the fluid is compressed past rest density, so pressure visibly travels through a pool.', min:0, max:1, step:0.01},
      {k:'fluidFoam', n:'foam', d:'Speed-driven whitening. Fast loose particles wash toward white and read as spray and churn.', min:0, max:2, step:0.01},
    ],
  },
