...
const TUNING_SCHEMA = [
...

  {
    ...
    params: [
      ...
      {k:'fluidSettleEps', n:'settle below', d:'A fluid block whose FASTEST particle stays below this speed counts as calm; string enough calm ticks together and the block settles back into CA voxels. Raise it to put restless pools to sleep sooner; lower it if pools freeze mid-slosh.', min:0.05, max:20, step:0.05, u:'vox/s'},
      {k:'fluidWakeSpeed', n:'wake above', d:'Grid-node speed at an active/settled interface above which the neighbouring settled liquid is excited back into particles — the progressive wake that lets a splash spread across a still pool. Keep it well above the settle speed (the default is 4×): that gap is the hysteresis that stops blocks flickering between states.', min:0.1, max:50, step:0.1, u:'vox/s'},
      {k:'fluidSettleTicks', n:'settle ticks', d:'Consecutive calm ticks a block must accumulate before it settles into voxels. The floor of 8 is a hard requirement — it covers the page-materialization readback latency — so do not chase snappier settling below it.', min:8, max:600, step:1, int:true},
      {k:'fluidStainRate', n:'stain rate', d:'Per-second chance that excited fluid touching a solid cell stains it — the MPM counterpart of the CA liquids’ staining, so churning blood paints the walls it slaps against. 0 turns particle staining off entirely.', min:0, max:30, step:0.5, u:'/s'},
    ],
  },
