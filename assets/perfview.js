...
let pvLive = { samples: [], connected: false, status: 'not connected', raf: 0, dirty: false };
...
function pvLegend(items){
  ...
  for (const it of items){
    l.append(pvEl('span', { class:'pv-legend-item' },
...
function pvBreakdownSection(V, T){
  ...
  sec.append(pvBars(T.gpu.map(g => ({
    label:g.label, ms:g.ms, side:'gpu', col:PV_SEQ[3], why: whyFor(g.id),
    tip: (pvNodeById(g.id)||{}).note || '' })), { total:T.wallMs }));
  ...
  if (T.waitMs > 0) cpuRows.push({
    ...
    tip:'The CPU parked in front of a full frame queue. Large here means the '
...
function pvConnect(attempt){
  ...
  pvSock.onmessage = ev => {
    ...
    if (m.v !== 2) return;              // v1 stage messages belong to the Engine tab
  ...
  pvSock.onclose = () => {
    ...
    if (attempt < 12){
      pvLive.status = 'waiting for the game to come up… (' + (attempt+1) + '/12)';
      setTimeout(() => pvConnect(attempt+1), 1000);
    } else {
      pvLive.status = 'disconnected — is the game running with --telemetry?';
    }
    renderPerformance();
  };
}
