// WIND SLOPE-FIELD DEBUG OVERLAY — the arrows (F4).
//
// docs/RESEARCH_wind.md §4.8. An instanced arrow per point of a lattice around
// the camera, each one oriented and coloured by the wind AT that point.
//
// THE WHOLE POINT is that it calls `windAt` — the same function in common.wgsl
// that the grass sway calls (DESIGN.md §12 invariant 2). A visualiser with its
// own copy of the field would be a picture of a DIFFERENT wind, agreeing with
// the world only until someone edited one of the two, and it would be exactly
// as convincing while wrong. So: no field code lives in this file. It samples,
// orients, colours and gets out of the way.
//
// ZERO CPU WORK PER ARROW. There is no arrow buffer and nothing is uploaded:
// the vertex shader derives its lattice point from the instance index and
// R.camPos alone, so the whole overlay is one Draw(vertices, instances) with
// no writes at all. And zero cost when off — main.cpp skips the draw entirely
// rather than drawing it transparent.
//
// WHY QUADS AND NOT LineList, the same answer debug_lines.wgsl gives: WebGPU
// guarantees no line width past 1 px, so a real line list aliases up close and
// vanishes at any distance. Each of the three segments of an arrow (shaft plus
// two head barbs) is expanded into a thin CAMERA-FACING quad whose thickness
// is set in world units and floored in screen space.
...
// WindDebugArrowsPerAxis (src/sim/wind.h), which is what decides how many
...
fn arrowsPerAxis() -> i32 {
  let half = clamp(i32(TUNE_WIND_DBG_RADIUS / TUNE_WIND_DBG_SPACING), 0, 24);
  return 2 * half + 1;
}
...
// cells/s. 240 = 24 m/s = a gale, so an ordinary breeze sits in the blue-green
// half of the ramp and there is headroom left to SEE a storm arrive.
const WIND_DBG_FULL_SCALE : f32 = 240.0;

@vertex
fn vsArrow(@builtin(vertex_index) vi : u32,
  ...
  let sp = max(TUNE_WIND_DBG_SPACING, 1.0);
  ...
  let far = 1.0 - smoothstep(TUNE_WIND_DBG_RADIUS * 0.55,
                             TUNE_WIND_DBG_RADIUS, dist);
...
@fragment
fn fsArrow(in : VSOut) -> @location(0) vec4f {
