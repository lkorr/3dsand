...
Same seed+tick+inputs → same world hash everywhere. Integer-only sim math (no f32 in CA). Stateless counter-based RNG: `hash3(seed,tick,cellIndex)`. No scheduling-dependent outcomes (no atomics-CAS, no subgroup ops). Write reach ≤1 cell. **Gate:** `--selftest` runs sim twice, compares hashes. Pinned hash: `dc666ada` (was `58b27f33` until WP5b: settled liquid carries MPM boundary mass, the excite ceiling stopped being charged against spawns, the settle exclusion became a column rule, and liquid gained a minimum film — 2026-08-25; `7b01cfd8` before WP5 flipped sim.fluidExciteMode to 1; `7cfa2420` before the CA liquid four-defect fix). If it moves, you broke the sim — stop and report.
