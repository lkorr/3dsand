...
| `assets/prefabs/`, `assets/mobs/` | `.vox` art + mob `.json` sidecars. A sidecar's `anatomy` block is what is under the skin by depth (skin / flesh / muscle / bone), baked into the `.vox` by `assets/editor/anatomy.js` (`node scripts/anatomize_mob.mjs <mob>`, or the Models tab's anatomy row); `scripts/test_anatomy.mjs` pins the human to its recipe. Mob voxel material ids must stay <= 127 (128..255 is the art palette) |
...
- **The voxel buffer is a PAGE POOL, not a dense array** (`docs/PLAN_page_table.md`). `pageTable[slot]` is a page index or an `EMPTY`/`UNIFORM(mat)` sentinel; 44.7 MiB resident vs 512 MiB dense in the harness window. Address voxels ONLY through `voxWordAt`/`voxWordIndex`/`voxStore` (WGSL) or `World::PageOffsetOfSlot` (C++) — any other subscript reads another chunk's memory. An index used as an **identity** (hash key, RNG key, claim slot) is the SLOT index; only a memory address is the PAGE index. The table is derived data: not hashed, not saved. Pool exhaustion is a **fatal abort**. **Paged is the DEFAULT residency**; `--residency dense` is the identity map and the only live differential oracle. Sentinels are `EMPTY`, `UNIFORM(mat)` and **`JITTER(mat)`** (one material + worldgen's positional palette variant, §9 of the plan) — the last is what compresses buried bulk, since a stone chunk's per-cell `% 3` variant makes it non-uniform. Size or test residency with `--autofly-hard` (adversarial: diagonal + descent, fixed tick schedule), never with a standing player or the sky-heavy harness window — both under-report by 2x. The adversarial descent that measured 32,365/32,768 slots resident (98.8%) and exhausted the pool now settles at **14,697 (-55%)** and runs clean. **`kPoolPages` is DERIVED, never a literal: `kNumChunks + kPageRetireCeiling`** (2026-08-30) — one page per chunk slot plus every page the retire queue can hold out of the free list (`kPageFreeProbesPerTick × kPageRetireTicks` = 2,048). A pool of exactly `kNumChunks` aborts at 93.75% residency, not 100%, because pages leave the free list by two doors and the "one page per slot, so dense is enough" argument only counts one; that cost two crashes and one wrong "structurally impossible" diagnosis. Change `kWorldN` and the pool follows — but a `static_assert` in `world.h` refuses `kWorldN = 1024` (4.03 GiB in one binding, over Vulkan's 4 GiB `maxStorageBufferRange` ceiling), so doubling the window needs the buffer SPLIT first, not just the constant. **Neither adversarial harness reaches the exhaustion band**: `--autofly-hard` peaks at 54.2% and a full `--selftest` at 72.1%, because both stress STREAMING; what fills the pool is ACTIVITY (a world-wide wake converts the free `PT_EMPTY` sky sentinels — about half the window — into real pages). Diagnose with `SANDVOX_PT_AUDIT=1` (per-tick page-accounting recount, aborts on the first imbalance); `Alloc()` failure now prints a verdict naming which of genuine-demand / retire-starvation / bookkeeping-leak / lost-pages fired.
